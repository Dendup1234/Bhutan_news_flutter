...
class _LifeStyleTabState extends State<LifeStyleTab> {
  ...
  Future<void> scrapeNews() async {
    ...
    final response = await http
        .get(Uri.parse("https://kuenselonline.com/category/life-style/"));
    if (response.statusCode == 200) {
      ...
      final highlightedTitle =
          highlightedNews?.querySelector("h5.mt-0.post-title")?.text.trim() ??
              "No Title";
      final highlightedDate =
          highlightedNews?.querySelector("p.post-date")?.text.trim() ??
              "No Date";
      final highlightedDescription =
          highlightedNews?.querySelector("p.post-date + p + p")?.text.trim() ??
              "No Description";
      ...
      if (highlightedImageUrl.isNotEmpty &&
          highlightedImageUrl.startsWith('/')) {
        highlightedImageUrl = 'https://kuenselonline.com/category/business/' +
            highlightedImageUrl;
      }
      final highlightedNewsUrl = highlightedNews
              ?.querySelector("h5.mt-0.post-title a")
              ?.attributes['href'] ??
          "";
      ...
      for (var article in mediaNews) {
        final title =
            article.querySelector("h5.mt-0.post-title")?.text.trim() ??
                "No Title";
        final date =
        ...
        final newsUrl =
            article.querySelector("h5.mt-0.post-title a")?.attributes['href'] ??
                "";
